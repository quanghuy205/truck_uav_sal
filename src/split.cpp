...
double CalculateDroneCost(int j, int i, vector <double> DroneCost,vector <int> T) {
    double cost = 0;
    for (int k = j+1; k < i; k++) {
        cost += DroneCost.at(T.at(k));    
    }
    return cost;
}

//calculate UB2 (running algorithm split() without bouding mechanism and new dominance rule)
double calculateUB2 (vector<vector<double> > VehicleCost, vector<double> DroneCost, int NumberOfCustomer, vector <int> T) {
   int i = 0;
    ...
    // for (int j = 0; j <= n+1; j++) {
    //     cout << "ListUB2 " << "L(" << j << ") :";
    //     if (!ListUB2.at(j).empty()){
    //         for (int v = 0; v < ListUB2.at(j).size(); v++) {
    //             cout << "(" << ListUB2.at(j).at(v).first << "," <<  ListUB2.at(j).at(v).second<< ")" << ", ";
    //         }
    //     }    
    //     cout << endl;  
    // }
...
double calculateUB3 (vector<vector<double> > VehicleCost, vector <double> DroneCost, int NumberOfCustomer, vector <int> T) {
    ...
    while (i < n + 1) {
        ...
        cout << " (" << UB3.vehicle_cost << "," << UB3.drone_cost << ")";
    }
    cout << endl;
...
double *calculateLB_veh(vector<vector<double> > VehicleCost, vector <double> DroneCost, int NumberOfCustomer, vector <int> T) {
    ...
    double *LB_veh = new double[NumberOfCustomer+2];
...
double *calculateLB_tot(vector<vector<double> > VehicleCost, vector <double> DroneCost, int NumberOfCustomer, vector <int> T) {
    double *LB_tot = new double[NumberOfCustomer+2];
...
void split(vector <int> T, vector <int> &T_vehicle, vector <int> &T_drone, int n, vector <vector<double> > VehicleCost, vector <double>  DroneCost, double &UB1) {   
   ...
   double UB2 = calculateUB2(VehicleCost,DroneCost,n,T);

    double UB3 = calculateUB3(VehicleCost,DroneCost,n,T);
    double UB = min(min(UB1,UB2),UB3); // UB = min (UB1,UB2, UB3)
    cout << "LB_veh = ";
    double *ptr_LB_veh = calculateLB_veh(VehicleCost,DroneCost,n,T);
    for (int i = 0; i <= n + 1; i++) {
        cout << ptr_LB_veh[i] << " " ;
    }
    cout << endl;
    cout << "LB_tot = ";
    double *ptr_LB_tot = calculateLB_tot(VehicleCost,DroneCost,n,T);
    for (int i = 0; i <= n + 1; i++) {
        cout << ptr_LB_tot[i] << " " ;
    }
    cout << endl;
  ...
  //  cout << "list[n+1][0] " << ListOfLabel[n+1][0].vehicle_cost << endl;
    ...
    for (int v = 1; v < ListOfLabel[n+1].size(); v++) {
        ...
        if (tmp_MinLabel > max(ListOfLabel[n+1][v].vehicle_cost, ListOfLabel[n+1][v].drone_cost)) {
            ...
            if (tmp_MinLabel == max(ListOfLabel[n+1][v].vehicle_cost, ListOfLabel[n+1][v].drone_cost)) {
                if(min(ListOfLabel[n+1][best_LabelIndex].vehicle_cost,ListOfLabel[n+1][best_LabelIndex].drone_cost) > min(ListOfLabel[n+1][v].vehicle_cost, ListOfLabel[n+1][v].drone_cost)) {
                    tmp_MinLabel = max(ListOfLabel[n+1][v].vehicle_cost, ListOfLabel[n+1][v].drone_cost);
                    best_LabelIndex = v;
                }
            }
    ...
    UB1 = max(ListOfLabel[n+1][best_LabelIndex].vehicle_cost,ListOfLabel[n+1][best_LabelIndex].drone_cost);
    cout << "UB1" << UB1;
